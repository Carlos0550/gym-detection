...
class GymOnboardingResponse(BaseModel):
    ...
    owner_name: str = Field(examples=["John Doe"])
    owner_email: str = Field(examples=["john@doe.com"])
    owner_access_token: str = Field(examples=["eyJhbGciOiJIUzI1NiIs..."])
...
class GymOnboardingRequest(BaseModel):
    ...
    owner_name: str = Field(min_length=2, max_length=255, examples=["John Doe"])
